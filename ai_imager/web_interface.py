...
def API(
    ...
    @api_config.imager_error_handler()
    @app.route("/v1/image/analyze", methods=["GET"])
    def analyze_form():
        """Image-to-prompt form"""
        return render_template("analyze.html")

    @api_config.imager_error_handler()
    @app.route("/v1/image/analyze/generate", methods=["POST"])
    def analyze_image():
        """Generate a prompt from an uploaded image"""
        files = api_config.get_from_file("path_to_image")
        if not files.get("path_to_image"):
            return jsonify({"prompt": None, "error": api_config.incomplete_form_msg}), 400
        features = openai._analyze_image_features(files["path_to_image"])
        result = openai.image_to_prompt(**files)
        if isinstance(result, list):
            result = result[0] if result else "Could not generate a prompt."
        if isinstance(result, str) and not result.startswith("Failed"):
            return jsonify({
                "prompt": result,
                "palette": features.get("hex_colors", []),
                "error": None
            })
        return jsonify({"prompt": None, "error": result or "Failed to analyse image."}), 501
    ...
    @app.after_request
    def pretty_json_for_browser(response):
        """When a browser navigates directly to a JSON endpoint,
        return a branded pretty-print page instead of raw JSON."""
        import json as _json
        is_browser_nav = request.headers.get("Sec-Fetch-Mode") == "navigate"
        is_json = response.content_type.startswith("application/json")
        if is_browser_nav and is_json:
            try:
                data = _json.loads(response.get_data(as_text=True))
                pretty = _json.dumps(data, indent=2, ensure_ascii=False)
                html = render_template("json_pretty.html",
                                       json_text=pretty,
                                       endpoint=request.path,
                                       status=response.status_code)
                return make_response(html, response.status_code)
            except Exception:
                pass
        return response
